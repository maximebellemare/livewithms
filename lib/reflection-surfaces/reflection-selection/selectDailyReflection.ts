...
function buildContinuityCard(input: ReflectionSelectionInput): ReflectionSurfaceCard | null {
  ...
  if (input.lifecycleStage === "returning") {
    return {
      ...
      title: "A return can still be enough",
      body: "Coming back after a harder stretch can still add a little steadiness.",
      source: "continuity",
      relatedMetrics: [],
      tone: "light",
      confidence: "light",
  ...
  if (input.adaptiveState.primary === "WITHDRAWN") {
    return {
      ...
      body: "Even a brief note can make the week feel a little easier to follow.",
      source: "continuity",
      relatedMetrics: [],
      tone: "light",
      confidence: "light",
