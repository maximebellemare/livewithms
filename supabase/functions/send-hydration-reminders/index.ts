...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!profiles || profiles.length === 0) {
      return new Response(JSON.stringify({ sent: 0, message: "No users with push enabled" }), {
    ...
    for (const profile of profiles) {
      ...
      const currentIntake = entry?.water_glasses ?? 0;

      // Only remind if intake is below 50% of goal by mid-afternoon
      if (currentIntake >= Math.ceil(goal * 0.5)) {
        skipped++;
        continue;
      }
    ...
    console.log(`[hydration-reminders] sent=${sent} skipped=${skipped} failed=${failed}`);

    return new Response(JSON.stringify({ sent, skipped, failed, date: today }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("[hydration-reminders] error:", message);
