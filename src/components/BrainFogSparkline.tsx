...
interface BrainFogSparklineProps {
  entries: Entry[];
}

/** Lower brain fog is better (0-10 scale) */
...
export default function BrainFogSparkline({ entries }: BrainFogSparklineProps) {
  ...
  const points = days.map((date) => {
    ...
    return { date, value: entry?.brain_fog ?? null };
  });
  ...
  const toSvgY = (v: number) => PAD + (v / 10) * (H - PAD * 2);
  ...
  return (
    <div
      ...
      onClick={() => navigate("/insights", { state: { heatmapMetric: "brain_fog" } })}
      role="button"
      tabIndex={0}
    >
      <div className="flex-shrink-0">
        ...
        <div className="flex items-baseline gap-1">
          <span
            ...
            style={{ color: avg !== null ? brainFogColor(avg) : "hsl(var(--muted-foreground))" }}
          >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between mt-0.5 px-0.5">
          {days.map((date) => {
            const hasData = byDate[date]?.brain_fog != null;
            return (
              <span
                key={date}
