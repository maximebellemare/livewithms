...
/** Lower pain is better (0-10 scale) */
...
export default function PainSparkline({ entries }: PainSparklineProps) {
  ...
  const points = days.map((date) => {
    ...
    return { date, value: entry?.pain ?? null };
  });
  ...
  // For pain, going DOWN is good (↓ = improving)
  const trend =
  ...
  return (
    <div
      ...
      onClick={() => navigate("/insights", { state: { heatmapMetric: "pain" } })}
      role="button"
      tabIndex={0}
    >
      <div className="flex-shrink-0">
        ...
        <div className="flex items-baseline gap-1">
          <span
            ...
            style={{ color: avg !== null ? painColor(avg) : "hsl(var(--muted-foreground))" }}
          >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between mt-0.5 px-0.5">
          {days.map((date) => {
            const hasData = byDate[date]?.pain != null;
            return (
              <span
                key={date}
