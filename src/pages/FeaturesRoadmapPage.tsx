...
const FeaturesRoadmapPage = () => {
  ...
  const FeatureCard = ({ slug, title, icon }: { slug: string; title: string; icon: React.ReactNode }) => (
    <button
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="mx-auto max-w-lg px-4 space-y-8">
        <section>
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Phase 2 — Next Up</h2>
          <div className="space-y-2">
            {phase2.map(([slug, f]) => (
              <FeatureCard key={slug} slug={slug} title={f.title} icon={f.icon} />
            ))}
          </div>
        </section>
        <section>
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Phase 3 — Future</h2>
          <div className="space-y-2">
            {phase3.map(([slug, f]) => (
              <FeatureCard key={slug} slug={slug} title={f.title} icon={f.icon} />
            ))}
