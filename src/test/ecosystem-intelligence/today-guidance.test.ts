...
describe("ecosystem intelligence today guidance", () => {
  it("keeps the guidance surface coordinated and low-pressure", () => {
    ...
    expect(result.body.toLowerCase()).toMatch(/calm|simpler|surface|quieter|support/);
    expect(result.body.toLowerCase()).not.toMatch(/everything at once|act now|don't miss/);
  });
});
