...
describe("cognitive support today guidance", () => {
  it("adds calm cognitive support without performance pressure", () => {
    const result = buildTodayGuidance(entry, null, "2026-05-15", adaptiveProfile, memory, null);

    expect(result.body.toLowerCase()).toMatch(/attention|clearer|brain fog|shorter steps|fewer choices/);
    expect(result.body.toLowerCase()).not.toMatch(/score|brain performance|brain power|push through/);
  });
});
