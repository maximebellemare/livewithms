...
describe("learning ecosystem today guidance", () => {
  it("adds calmer learning cues without fear amplification", () => {
    const result = buildTodayGuidance(entry, null, "2026-05-15", adaptiveProfile, memory, null);

    expect(result.body.toLowerCase()).toMatch(/learning|topic|summary|useful/);
    expect(result.body.toLowerCase()).not.toMatch(/worsening|worry about|keep reading|fight harder/);
  });
});
