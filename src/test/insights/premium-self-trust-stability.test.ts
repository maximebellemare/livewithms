...
describe("premium self trust stability", () => {
  it("builds self-trust support without therapy or self-help framing", () => {
    const entries = Array.from({ length: 21 }, (_, index) =>
      buildEntry(getDateDaysAgo(index), {
        fatigue: index < 8 ? 4 : 3,
        stress: index < 8 ? 4.2 : 3,
        mood: index < 8 ? 2.4 : 3.1,
        symptom_tags: index < 8 ? ["fatigue", "dizziness"] : ["fatigue"],
        notes:
          index < 5
    ...
    const result = derivePremiumSelfTrustStabilitySummary(entries, buildSnapshot());
    const combined = [
      result.atAGlance,
      ...result.selfTrustRebuildingSupport,
      ...result.reducedHypervigilanceSupport,
      ...result.smallerSteadinessSupport,
      result.continuityNote,
    ].join(" ").toLowerCase();

    expect(result.hasEnoughData).toBe(true);
    expect(combined).toMatch(/trust|predictable|monitor|steadier|gentleness/);
  ...
  it("keeps low-energy summaries shorter and uses a calm fallback", () => {
    const fallback = derivePremiumSelfTrustStabilitySummary(
    ...
    const shorter = derivePremiumSelfTrustStabilitySummary(
      Array.from({ length: 12 }, (_, index) =>
        buildEntry(getDateDaysAgo(index), {
          fatigue: 4,
          stress: 4.2,
          mood: 2.5,
          notes: index < 3 ? "I keep scanning symptoms and do not trust what my body is doing." : null,
        }),
      ),
      buildSnapshot(),
      { lowEnergyMode: true },
    ...
    expect(shorter.selfTrustRebuildingSupport.length).toBeLessThanOrEqual(1);
    expect(shorter.smallerSteadinessSupport.length).toBeLessThanOrEqual(1);
  });

  it("gates access only for active premium users with the feature enabled", () => {
    expect(canAccessPremiumSelfTrustStability(true, true)).toBe(true);
    expect(canAccessPremiumSelfTrustStability(true, false)).toBe(false);
    expect(canAccessPremiumSelfTrustStability(false, true)).toBe(false);
  });
});
