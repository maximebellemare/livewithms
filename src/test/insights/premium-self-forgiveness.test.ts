...
describe("premium self forgiveness", () => {
  it("builds calm self-forgiveness support without therapy or self-help framing", () => {
    const entries = Array.from({ length: 21 }, (_, index) =>
      buildEntry(getDateDaysAgo(index), {
        ...
        notes:
          index < 6
            ? "I am being too hard on myself and feel guilty that I should be doing more."
            : null,
      }),
    );

    const result = derivePremiumSelfForgivenessSummary(entries, buildSnapshot());
    const combined = [
      result.atAGlance,
      ...result.gentleSelfForgivenessSupport,
      ...result.guiltDecompressionSupport,
      ...result.smallerCapacitySupport,
    ...
    expect(combined).toMatch(/gentler|guilt|pressure|capacity|self|rest/);
  ...
  it("keeps low-energy summaries shorter and uses a calm fallback", () => {
    const fallback = derivePremiumSelfForgivenessSummary(
    ...
    const shorter = derivePremiumSelfForgivenessSummary(
      Array.from({ length: 12 }, (_, index) =>
        buildEntry(getDateDaysAgo(index), {
          fatigue: 4,
          stress: 4,
          mood: 2.5,
          notes: "I feel guilty and like I am not enough lately.",
        }),
      ),
      buildSnapshot(),
      { lowEnergyMode: true },
    ...
    expect(shorter.gentleSelfForgivenessSupport.length).toBeLessThanOrEqual(1);
    expect(shorter.guiltDecompressionSupport.length).toBeLessThanOrEqual(1);
    expect(shorter.smallerCapacitySupport.length).toBeLessThanOrEqual(1);
  });

  it("gates access only for active premium users with the feature enabled", () => {
    expect(canAccessPremiumSelfForgiveness(true, true)).toBe(true);
    expect(canAccessPremiumSelfForgiveness(true, false)).toBe(false);
    expect(canAccessPremiumSelfForgiveness(false, true)).toBe(false);
  });
});
