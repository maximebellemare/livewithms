...
describe("ambient support today guidance", () => {
  it("adds ambient support cues without metric fixation", () => {
    const result = buildTodayGuidance(entry, null, "2026-05-15", adaptiveProfile, memory, null);

    expect(result.body.toLowerCase()).toMatch(/passive|signal|background|turn off|lighter/);
    expect(result.body.toLowerCase()).not.toMatch(/score|readiness|optimize|heart rate|biometric/);
  });
});
