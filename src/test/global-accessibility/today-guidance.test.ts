...
describe("global accessibility today guidance", () => {
  it("adds calmer accessibility cues without invasive or culturally sharp language", () => {
    const result = buildTodayGuidance(entry, null, "2026-05-15", adaptiveProfile, memory, null);

    expect(result.body.toLowerCase()).toMatch(/shorter|accessible|simple|calm|reading/);
    expect(result.body.toLowerCase()).not.toMatch(/we know you|follow instructions|act now|patient|compliance/);
  });
});
