...
describe("cross-platform continuity today guidance", () => {
  it("adds continuity support without cross-device pressure", () => {
    const result = buildTodayGuidance(entry, null, "2026-05-15", adaptiveProfile, memory, null);

    expect(result.body.toLowerCase()).toMatch(/device|surface|sync|later|familiar/);
    expect(result.body.toLowerCase()).not.toMatch(/urgent|every device|keep up everywhere|always-on/);
  });
});
